...
// NUVOTON: For no HTTP proxy implementation
#if 0
...
// NUVOTON: For including user configuration for model ID
 ...
 */
static const char g_aduModelId[] = ADUC_DEVICE_MODEL_ID;
...
static IOTHUB_CLIENT_TRANSPORT_PROVIDER GetIotHubProtocolFromConfig()
{
    ...
    if (ADUC_ConfigInfo_Init(&config, ADUC_CONF_FILE_PATH))
    {
        if (config.iotHubProtocol != NULL)
        {
            ...
            // NUVOTON: For no mqtt/ws code pulled in
#ifdef ADUC_ALLOW_MQTT_OVER_WEBSOCKETS
...
#endif
    // NUVOTON: For requiring CA certificate
    //          https://github.com/Azure/azure-iot-sdk-c/tree/main/certs
#if 1
    else if (
        connInfo->authType == ADUC_AuthType_SASToken
        && (iothubResult = ClientHandle_SetOption(*outClientHandle, OPTION_TRUSTED_CERT, certificates))
            != IOTHUB_CLIENT_OK)
    {
