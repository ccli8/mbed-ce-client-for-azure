...
// V4 and later.
// NUVOTON: According to import/update manifest schema, should default to inline if no value is provided.
//          https://docs.microsoft.com/en-us/azure/iot-hub-device-update/import-schema
//          https://json.schemastore.org/azure-deviceupdate-update-manifest-4.json
// FIXME: Default to inline?
#if 1
#define DEFAULT_STEP_TYPE "reference"
#else
#define DEFAULT_STEP_TYPE "inline"
#endif
...
// NUVOTON: For potential stack overflow
#define NU_FIX_STACKOVERFLOW        1
...
ADUC_Result _workflow_parse(bool isFile, const char* source, bool validateManifest, ADUC_WorkflowHandle* handle)
{
    ...
    *handle = NULL;
    ...
    if (updateAction != ADUCITF_UpdateAction_Cancel)
    {
        ...
        {
            // NUVOTON: For support only embedded manifest
            //          https://learn.microsoft.com/en-us/azure/iot-hub-device-update/update-manifest
#if 0
...
#else
            Log_Error("Support no downloadable update manifest file %s. Change to embedded manifest instead.", detachedManifestFileId);
...
bool workflow_set_workfolder(ADUC_WorkflowHandle handle, const char* format, ...)
{
    ...
    {
        // NUVOTON: For potential stack overflow
#if NU_FIX_STACKOVERFLOW
        char *buffer = (char *) malloc(WORKFLOW_RESULT_DETAILS_MAX_LENGTH);
#else
