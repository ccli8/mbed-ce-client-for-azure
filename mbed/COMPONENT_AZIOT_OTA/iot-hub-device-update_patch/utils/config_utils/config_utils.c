...
/* NUVOTON: For including user configuration like MANUFACTURER/MODEL */
 ...
 */
// NUVOTON: For user application override
//          See du-config.json:
//          https://learn.microsoft.com/en-us/azure/iot-hub-device-update/device-update-configuration-file
 __attribute__((weak))
...
// NUVOTON: For this port
    ...
    // Support only one agent
    config->agentCount = 1;
    config->agents = calloc(config->agentCount, sizeof(ADUC_AgentInfo));
    ...
    // The only one agent
    ADUC_AgentInfo* agent = config->agents + 0;

    // du-config.json: agents[0].name: Agent name
    if (mallocAndStrcpy_s(&(agent->name), ADUC_AGENT_NAME) != 0)
    {
        goto done;
    }

    // du-config.json: agents[0].runas: Not significant in this port
    if (mallocAndStrcpy_s(&(agent->runas), "adu") != 0)
    {
        goto done;
    }

    // du-config.json: agents[0].connectionSource.connectionType: Support only connection string
    if (mallocAndStrcpy_s(&(agent->connectionType), "string") != 0)
    {
        goto done;
    }

    // du-config.json: agents[0].connectionSource.connectionData: Device or module connection string
    if (mallocAndStrcpy_s(&(agent->connectionData), ADUC_DEVICE_CONNECTION_STRING) != 0)
    {
        goto done;
    }

    // du-config.json: agents[0].manufacturer: AzureDeviceUpdateCore:4.ClientMetadata:4 
    if (mallocAndStrcpy_s(&(agent->manufacturer), ADUC_DEVICEPROPERTIES_MANUFACTURER) != 0)
    {
        goto done;
    }

    // du-config.json: agents[0].model: AzureDeviceUpdateCore:4.ClientMetadata:4 
    if (mallocAndStrcpy_s(&(agent->model), ADUC_DEVICEPROPERTIES_MODEL) != 0)
    {
        goto done;
    }

    // du-config.json: DeviceInfo: manufacturer/model
    if (mallocAndStrcpy_s(&(config->manufacturer), ADUC_DEVICEINFO_MANUFACTURER) != 0
        || mallocAndStrcpy_s(&(config->model), ADUC_DEVICEINFO_MODEL) != 0)
    {
        goto done;
    }

    // du-config.json: compatPropertyNames
    if (mallocAndStrcpy_s(&(config->compatPropertyNames), ADUC_COMPAT_PROPERTY_NAMES) != 0)
    {
        goto done;
    }

    // du-config.json: iotHubProtocol: Support only "mqtt"
    if (mallocAndStrcpy_s(&(config->iotHubProtocol), "mqtt") != 0)
    {
        goto done;
    }
#endif
...
void ADUC_ConfigInfo_UnInit(ADUC_ConfigInfo* config)
{
    ...
    free(config->model);
    // NUVOTON: For memory leak
#if 1
    free(config->compatPropertyNames);
#endif
