...
// NUVOTON: Unnecessary for no downloadHandler implementation
#if 0
...
// NUVOTON: Unnecessary for no file system implementation
#if 0
...
#else
static rtos::Mutex s_workflow_mutex;

static inline void s_workflow_lock(void)
{
    s_workflow_mutex.lock();
}

static inline void s_workflow_unlock(void)
{
    s_workflow_mutex.unlock();
}
#endif
 ...
 */
// NUVOTON: Unnecessary for no sandbox environment
#if 0
...
void ADUC_Workflow_HandleStartupWorkflowData(ADUC_WorkflowData* currentWorkflowData)
{
    ...
    else
    {
        ...
        // NUVOTON: Fix for C++ strong typing
#if 0
        int desiredAction = workflow_get_action(currentWorkflowData->WorkflowHandle);
#else
        ADUCITF_UpdateAction desiredAction = workflow_get_action(currentWorkflowData->WorkflowHandle);
#endif
...
void ADUC_Workflow_HandleUpdateAction(ADUC_WorkflowData* workflowData)
{
...
#else
    ADUCITF_UpdateAction desiredAction = workflow_get_action(workflowData->WorkflowHandle);
#endif
    ...
    //
    // NUVOTON: For goto cannot cross variable definition
#if 0
    ADUC_Result isInstalledResult = ADUC_Workflow_MethodCall_IsInstalled(workflowData);
#else
    ADUC_Result isInstalledResult; isInstalledResult = ADUC_Workflow_MethodCall_IsInstalled(workflowData);
#endif
    if (isInstalledResult.ResultCode == ADUC_Result_IsInstalled_Installed)
    {
...
#else
    ADUCITF_WorkflowStep nextStep; nextStep = AgentOrchestration_GetWorkflowStep(desiredAction);
#endif
...
#else
    ADUC_MethodCall_Data* methodCallData; methodCallData = (ADUC_MethodCall_Data*) calloc(1, sizeof(ADUC_MethodCall_Data));
#endif
...
void ADUC_Workflow_WorkCompletionCallback(const void* workCompletionToken, ADUC_Result result, bool isAsync)
{
    ...
    ADUCITF_WorkflowStep currentWorkflowStep = workflow_get_current_workflowstep(workflowData->WorkflowHandle);
#else
    ADUCITF_WorkflowStep currentWorkflowStep; currentWorkflowStep = workflow_get_current_workflowstep(workflowData->WorkflowHandle);
#endif
...
// NUVOTON: For goto cannot cross variable definition
    ...
    const ADUC_WorkflowHandlerMapEntry* entry = GetWorkflowHandlerMapEntryForAction(currentWorkflowStep);
#else
    const ADUC_WorkflowHandlerMapEntry* entry; entry = GetWorkflowHandlerMapEntryForAction(currentWorkflowStep);
#endif
    if (entry == NULL)
    {
...
{
    ADUC_WorkflowData* workflowData = methodCallData->WorkflowData;
    // NUVOTON: Fix type error, caught in C++ but not in C
#if 0
    ADUC_WorkflowHandle* workflowHandle = workflowData->WorkflowHandle;
#else
    ADUC_WorkflowHandle workflowHandle = workflowData->WorkflowHandle;
