...
ADUC_Result PrepareStepsWorkflowDataObject(ADUC_WorkflowHandle handle)
{
    ...
    if (childWorkflowCount != stepCount)
    {
        ...
        for (unsigned int i = 0; i < stepCount; i++)
        {
            ...
            {
                // NUVOTON: Detached update manifest (reference step) is not supported due to:
                //          1. ExtensionManager::Download() not implemented for requiring file system
                //          2. No file system implementation
...
static ADUC_Result StepsHandler_Download(const tagADUC_WorkflowData* workflowData)
{
    ...
    // NUVOTON: For no sandbox implementation
#if 0
